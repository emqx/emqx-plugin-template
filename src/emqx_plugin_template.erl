...
on_client_disconnect_redis(ClientInfo = #{clientid := ClientId}, ReasonCode, ConnInfo, _Env) ->
  RedisPool = {single,emqx_backend_redis_pool1},
  io:format("Redis called\n"),
  emqx_backend_redis:on_client_disconnected(#{clientid => ClientId}, ReasonCode, ConnInfo, {undefined,RedisPool}),
  io:format("Redis executed\n").

on_client_disconnect_mysql(ClientInfo = #{clientid := ClientId}, ReasonCode, ConnInfo, _Env) ->
  MysqlPool = emqx_backend_mysql_pool1,
  io:format("Mysql called\n"),
